...
describe("JetQueue", () => {
  ...
  beforeEach(() => {
    queue = new JetQueue({ concurrency: 2 });
  });
  ...
  describe("Concurrency Control", () => {
    test("should respect concurrency limit", async () => {
      ...
      const queue = new JetQueue({ concurrency: 2 });
  ...
  describe("Job States", () => {
    test("should track pending → running → completed", async () => {
      ...
      const blockPromise2 = new Promise((resolve) => {
      ...
      const trackedJobPromise = new Promise((resolve) => {
        resolveTrackedJob = resolve;
      });
